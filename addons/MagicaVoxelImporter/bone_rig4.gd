...
func _set_upgrade_3to4(value):
	if !value || !Engine.is_editor_hint() || self is Area:
		return
	var node = Node3D.new()
	node.script = script_changed
...
func _collect_bones():
	...
	for i in len(vertices):
		...
		while b < weights_sz:
			...
			if bw < 0.9999 && bw > 0.0001:
				print("blendfactor: ", bw, " ", b, " ", qp.transform.origin)
			weights[b_i + b] = bw
			b += 1
		if len(bone_names) > 1:
			bone_names.sort()
...
func _calc_bone_weight(area: Area3D, enabled_shapes: Dictionary, shape: CollisionShape3D, state: PhysicsDirectSpaceState3D, origin: Vector3) -> float:
	...
	var RAY_LEN = 1000.0
	var bone_name = _shape_name_to_bone_name(shape.name)
	print("exclude: ", bone_name, exclude)
	print("notexclude: ", enabled_shapes[bone_name][0].get_rid())
	var ray_start = shape.global_transform.origin
	var ray_end = ray_start + ray.normalized() * RAY_LEN
	# ray points outward from the center of the shape from the current voxel
	...
	if intersect:
		printerr("intersect: ", intersect, " ", shape.shape.get_rid() == intersect.rid)
